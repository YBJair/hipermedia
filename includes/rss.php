...
$channel_node->appendChild($xml->createElement("title", "Pictures and Images")); //title
$channel_node->appendChild($xml->createElement("description", "Tus fotos donde quieras, cuando quieras"));  //description
$channel_node->appendChild($xml->createElement("link", "http://localhost/hipermedia/")); //website link 
$channel_node->appendChild($xml->createElement("language", "es-es"));  //language
...
$results = $mysqli->query("SELECT idFoto, Titulo,Descrpcion,Fecha, Album, Fichero, FRegistro FROM fotos order by FRegistro DESC LIMIT 5");
...
if($results){ //we have records 
    while($row = $results->fetch_assoc()) //loop through each row
    {
      ...
      $link_node = $item_node->appendChild($xml->createElement("link", "http://localhost/hipermedia/imagen.php?id=$id")); //add link node under "item"
      
      //Unique identifier for the item (GUID)
      $guid_link = $xml->createElement("guid", "http://localhost/hipermedia/imagen.php?id=". $id);  
      ...
      //Published date
      $date_rfc = gmdate(DATE_RFC2822, strtotime($pdate));
